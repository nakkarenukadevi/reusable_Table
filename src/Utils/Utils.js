export const data = [
    {
        id: 1,
        name: "bhanu",
        city: "guntur",
        work: "software",
        address: "saradha colony",
        salary: 200000,
        surname: "naduri",
        father: "ravi"


    },
    {
        id: 2,
        name: "bhanuPrakash",
        city: "vijayawada",
        work: "devloper",
        address: "brodipet",
        salary: 80000,
        surname: "naduri",
        father: "ravi",
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 3,
        name: "bhanupriya",
        city: "kadapa",
        work: "software",
        address: "gandhi nagar",
        salary: 70000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 4,
        name: "Rohit",
        city: "guntur",
        work: "doctor",
        address: "saradha colony",
        salary: 200000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 5,
        name: "venkatrohith",
        city: "retur",
        work: "doctor",
        address: "near sivalayam",
        salary: 100000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 6,
        name: "jyothish",
        city: "retur",
        work: "backand devloper",
        address: "saradha colony",
        salary: 200000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 7,
        name: "renuka",
        city: "guntur",
        work: "software",
        address: "saradha colony",
        salary: 20000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 8,
        name: "renuka devi",
        city: "guntur",
        work: "front end devloper",
        address: "saradha colony",
        salary: 50000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 9,
        name: "archana",
        city: "jellamudi",
        work: "bank",
        address: "saradha colony",
        salary: 60000
        , surname: "naduri",
        father: "ravi"
    },
    {
        id: 10,
        name: "praveen",
        city: "jellamudi",
        work: "software",
        address: "saradha colony",
        salary: 90000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 11,
        name: "bhanu sai",
        city: "guntur",
        work: "software",
        address: "saradha colony",
        salary: 9000,
        surname: "naduri",
        father: "ravi"
    },
    {
        id: 12,
        name: "bhanu",
        city: "guntur",
        work: "software",
        address: "saradha colony",
        salary: 200000,
        surname: "naduri",
        father: "sankarrao"

    },

]
export const movieHeaders = [
    { label: "ID", dataType: "number", key: "id" },
    { label: "Title", dataType: "string", key: "title" },
    { label: "Director", dataType: "string", key: "director" },
    { label: "Release Year", dataType: "number", key: "release_year" },
    { label: "Genre", dataType: "string", key: "genre" },
    { label: "Duration (min)", dataType: "number", key: "duration" },
    { label: "IMDb Rating", dataType: "number", key: "rating" },
    { label: "Box Office (M USD)", dataType: "number", key: "box_office" },
    { label: "Language", dataType: "string", key: "language" },
    { label: "Country", dataType: "string", key: "country" },
    { label: "Budget (M USD)", dataType: "number", key: "budget" }

]
export const moviedata = [
    {
        id: 1,
        title: "Inception",
        director: "Christopher Nolan",
        release_year: 2010,
        genre: "Sci-Fi",
        duration: 148,
        rating: 8.8,
        box_office: 829.89,
        language: "English",
        country: "USA",
        budget: 160
    },
    {
        id: 2,
        title: "The Matrix",
        director: "The Wachowskis",
        release_year: 1999,
        genre: "Action",
        duration: 136,
        rating: 8.7,
        box_office: 463.5,
        language: "English",
        country: "USA",
        budget: 63
    },
    {
        id: 3,
        title: "Titanic",
        director: "James Cameron",
        release_year: 1997,
        genre: "Romance",
        duration: 195,
        rating: 7.8,
        box_office: 2187.46,
        language: "English",
        country: "USA",
        budget: 200
    },
    {
        id: 4,
        title: "The Dark Knight",
        director: "Christopher Nolan",
        release_year: 2008,
        genre: "Action",
        duration: 152,
        rating: 9.0,
        box_office: 1005.0,
        language: "English",
        country: "USA",
        budget: 185
    },
    {
        id: 5,
        title: "Avatar",
        director: "James Cameron",
        release_year: 2009,
        genre: "Sci-Fi",
        duration: 162,
        rating: 7.8,
        box_office: 2847.25,
        language: "English",
        country: "USA",
        budget: 237
    },
    {
        id: 6,
        title: "Pulp Fiction",
        director: "Quentin Tarantino",
        release_year: 1994,
        genre: "Crime",
        duration: 154,
        rating: 8.9,
        box_office: 213.93,
        language: "English",
        country: "USA",
        budget: 8
    },
    {
        id: 7,
        title: "The Godfather",
        director: "Francis Ford Coppola",
        release_year: 1972,
        genre: "Crime",
        duration: 175,
        rating: 9.2,
        box_office: 246.12,
        language: "English",
        country: "USA",
        budget: 6
    },
    {
        id: 8,
        title: "Gladiator",
        director: "Ridley Scott",
        release_year: 2000,
        genre: "Action",
        duration: 155,
        rating: 8.5,
        box_office: 460.5,
        language: "English",
        country: "USA",
        budget: 103
    },
    {
        id: 9,
        title: "Forrest Gump",
        director: "Robert Zemeckis",
        release_year: 1994,
        genre: "Drama",
        duration: 142,
        rating: 8.8,
        box_office: 678.22,
        language: "English",
        country: "USA",
        budget: 55
    },
    {
        id: 10,
        title: "Jurassic Park",
        director: "Steven Spielberg",
        release_year: 1993,
        genre: "Adventure",
        duration: 127,
        rating: 8.1,
        box_office: 1045.69,
        language: "English",
        country: "USA",
        budget: 63
    }
]
export const headers = [
    {
        label: "Name",
        key: "name",
        dataType: "string"

    },
    {
        label: "City",
        key: "city",
        dataType: "string"
    },
    {
        label: "Work",
        key: "work",
        dataType: "string"
    },
    {
        label: "Salary",
        key: "salary",
        dataType: "number"
    },
    {
        label: "Address",
        key: "address",
        dataType: "string"
    },
    {
        label: "Surname",
        key: "surname",
        dataType: "string"
    },
    {
        label: "Father",
        key: "father",
        dataType: "string"
    },

]